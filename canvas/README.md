...
2. 결제 완료 → `/auth/checkout-success` 가 Stripe 세션 검증 후 자동 로그인
3. webhook 이 Subscriber 레코드 저장
4. 동일 이메일로 구매한 블록들이 자동으로 Pro 표시
5. `/manage` 에서 본인 블록 조회 + 편집 + Stripe Customer Portal (해지·결제수단)

## 인증 흐름 (매직 링크)

이메일 기반 비밀번호 없는 인증. JWT를 HTTP-only 쿠키에 저장.

```
/manage 방문 (로그인 안 됨)
  ↓
이메일 입력 → POST /api/auth/request
  ↓
RESEND_API_KEY 설정됨 → 이메일 발송 (30분 유효 매직 토큰)
RESEND_API_KEY 없음 + dev → 응답에 debug_link 직접 노출
  ↓
사용자가 링크 클릭 → /auth/verify?token=…
  ↓
서버 검증 → 30일 세션 쿠키 (pc_session) 발급
  ↓
/manage 자동 리다이렉트 → 본인 블록·구독 표시
```

Stripe Checkout 완료자는 `/auth/checkout-success` 가 session_id 검증 후 자동 로그인 (매직 링크 불필요).

## 룸 편집 (`/manage/blocks/[id]`)

블록 소유자 + 로그인 시 접근 가능. 필드별 권한:
...
PATCH `/api/blocks/[id]` 가 세션·소유권·Pro 상태를 모두 검증.
